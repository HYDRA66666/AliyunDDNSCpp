...
- force_update: 即使ip没有变化也强制更新解析记录。目前，如果解析记录名、记录值完全一致，阿里云api将返回400错误，仅用于修改config文件后刷新程序。
- urls: 获取本机公网ip的url，需要网址返回纯文本形式的ip，不能有任何格式修饰符。一般情况下保持不变即可。
  - ipv4: 中国大陆用户需将此值更改为 ``https://api.ipify.cn``
- domains: 待更新解析记录的域名

2. 保存文件，再次双击运行程序，观察输出。
3. 在windows计划任务管理器中设置定时任务，任务内容未执行本程序，无需附带任何参数。
